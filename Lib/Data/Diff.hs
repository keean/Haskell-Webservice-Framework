...
editDistance a b = last (if lab == 0
    ...
    oneDiag c d diagAbove diagBelow = thisdiag where
    --oneDiag (c :: [Char]) (d :: [Char]) diagAbove diagBelow = thisdiag where
...
mdiff :: (Num t, Num t1, Ix t, Ix t1, MArray a Int m) => a (t, t1) Int -> String -> String -> m ()
mdiff d s0 s1 = do
    writeArray d (0,0) 0
    foreach 1 s0 $ \x a -> do
