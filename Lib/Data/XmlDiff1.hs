...
module Lib.Data.XmlDiff1(xmlDiffExt,debugDiffExt,debugDiffExt1) where
...
costDelete _e = 1 -- length $ showElement e ""

costInsert :: XmlElement -> Int
costInsert _e = 1 -- length $ showElement e ""

costReplace :: XmlElement -> XmlElement -> Int
costReplace _a _b = 1 -- editDistance (showElement a "") (showElement b "")
-- costReplace x y = distString (showElement x "") (showElement y "")

costDeleteTree :: XmlElement -> Int -> Int
costDeleteTree _e _n = 1 -- (length $ showElement e "")

costInsertTree :: XmlElement -> Int -> Int
costInsertTree _e n = n -- (length $ showElement e "")

_costSwap :: Int -> Int -> Int
_costSwap _i _j = 1
------------------------------------------------------------------------------

xmlDiffExt :: DOM -> DOM -> (Int,Edit XmlElement)
xmlDiffExt d d' = (\k k' -> postProcess $ toEdit k k' $ diffResult k k') (keyroots d) (keyroots d')

debugDiffExt :: DOM -> DOM -> (Int,[(Int,Int,Op XmlElement)])
debugDiffExt d d' = (\k k' -> toEdit k k' $ diffResult k k') (keyroots d) (keyroots d')

debugDiffExt1 :: DOM -> DOM -> Array (Int,Int) ZSResult
debugDiffExt1 d d' = (\k k' -> diffMatrix k k') (keyroots d) (keyroots d')
...
matrixST t0@(p0,_,_) t1@(p1,_,_) = do
    b@(_,_br) <- return $ (\(x0,x1) (y0,y1) -> ((x0,y0),(x1,y1))) (GHC.Arr.bounds p0) (GHC.Arr.bounds p1)
...
kdiff (d :: a (Int,Int) ZSResult) (t0,k0,l0) (t1,k1,l1) = do
    foreach k0 $ \x -> do
        lx <- return (l0!x)
        x0 <- return (lx-1)
        foreach k1 $ \y -> do
            ly <- return (l1!y)
            y0 <- return (ly-1)
            (fd :: a (Int,Int) ZSResult) <- newArray ((x0,y0),(x,y)) (0,[])
    ...
    _isKeyroot :: Int -> [Int] -> Bool
    _isKeyroot _ [] = False
    _isKeyroot i (kr0:krs)
        | i==kr0 = True
        | otherwise = _isKeyroot i krs
    ...
    _min3 :: MArray a ZSResult m => (Int,z) -> (Int,z) -> (Int,z) -> m (Int,z)
    _min3 i j k = do
        a <- min2 i j
        min2 a k
