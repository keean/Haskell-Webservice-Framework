...
module Lib.MetaSearch.Parser(MonadParser,Parser(..),Consumed(..),Reply(..),parse,parseResult,fmap,return,(>>=),mzero,mplus,
    ...
    consume,matchEnd,untilParserOrEnd,splitWords,trimParser) where
...
instance Applicative Parser where
    pure x = Parser (\cs -> Empty (Ok x cs))
    Parser mf <*> Parser mx = Parser (\cs -> case mf cs of
        Empty reply -> case reply of
            Ok f cs' -> case mx cs' of
                Empty reply' -> Empty $ handle f reply'
                Consumed reply' -> Consumed $ handle f reply'
            _ -> Empty Error
        Consumed reply -> Consumed $ case reply of
            Ok f cs' -> case mx cs' of
                Empty reply' -> handle f reply'
                Consumed reply' -> handle f reply'
            _ -> Error)
        where
            handle f reply = case reply of
                Ok x cs' -> Ok (f x) cs'
                _ -> Error
...
instance Alternative Parser where
    ...
    (Parser m) <|> (Parser n) = Parser $ \cs -> case m cs of
        Empty Error -> n cs
        other -> other

instance MonadPlus Parser where
    {-# INLINE mzero #-}
    mzero = Parser (\_ -> Empty Error)
    {-# INLINE mplus #-}
    ...
    matchParser p q = Parser (\cs -> case parse (tryParser p) cs of
        Empty _ -> case parse q cs of
            ...
            Consumed result -> Consumed $ case result of
                Ok _ cs' -> case parse (matchParser p q) cs' of
                    Consumed result' -> result' 
                    Empty result' -> result'
                Error -> Error
        consumed -> consumed)
...
isBlankText s@(c:cs) = case parse (stringNcs "&nbsp;") s of
    ...
    _ -> if isSpace c then isBlankText cs else False
...
trimText s@(c:cs) = if isBlankText s then [] else case parse (stringNcs "&nbsp;" ) s of
    ...
    _ -> if isSpace c then trimText cs else c:trimAlt cs where
        trimAlt :: String -> String
        trimAlt [] = []
        trimAlt s'@(_:_) = if isBlankText s' then [] else case parse (stringNcs "&nbsp;") s' of
            Consumed (Ok _ s'') -> ' ':trimText s''
            _ -> if isSpace c then ' ':trimText cs else c:trimAlt cs
