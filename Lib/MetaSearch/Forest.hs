...
httpPostToDocument headers loc args = case splitLocation loc of
    (host,port,file) -> do
         ...
         (do
             ...
             hFlush skt
             t <- hGetContents skt
             return (Just skt,t)) `Exception.catch` (\(e :: SomeException) -> do hClose skt; throw e)
...
maybeClose (Just h) = (hClose h) `Exception.catch` (\(e :: SomeException) -> hPutStr stderr $ shows e "\n")
maybeClose _ = return ()
...
getDocument' cookies i e = case e of
    (MkElem (Document,url,a)) -> (do
        ...
        `Exception.catch` (\(x :: SomeException) -> do
...
splitLocation loc = case parse getSocket loc of
    Consumed (Ok h f) -> case parse getHost h of
        Consumed (Ok h' p) -> (h', PortNumber $ fromInteger $ read p, f)
        _ -> (h,PortNumber 80,f)
    _ -> ("localhost", PortNumber 80, loc)
