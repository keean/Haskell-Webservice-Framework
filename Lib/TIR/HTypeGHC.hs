...
instance (TypeEq.TypeEq a b l,HContainsType' l r a t) => HContainsType (HCons b r) a t where
    hContainsType (_::HCons b r) a = hContainsType' (TypeEq.typeEq a (undefined::b)) (undefined::r) a
...
instance (HSubType r s t2,HContainsType r a t3,Logic.And t2 t3 t,Logic.Modal t) => HSubType r (HCons a s) t where
...
data TypeNotFound k = TypeNotFound k
...
instance (TypeEq.TypeEq a b t,HIndexOf' t l a n) => HIndexOf (HCons b l) a n where
    hIndexOf (_::HCons b l) a = hIndexOf' (TypeEq.typeEq a (undefined::b)) (undefined::l) a 
...
instance {-# OVERLAPPING #-} (HList (HCons e l),HReverseAcc acc l l') => HDeleteFst' (HCons e l) e acc l' where
    hDeleteFst' (HCons _ l) _ acc = hReverseAcc acc l
instance {-# OVERLAPPABLE #-} (HDeleteFst' l e (HCons e' acc) l') => HDeleteFst' (HCons e' l) e acc l' where
...
instance HDeleteAll HNil a HNil where
    hDeleteAll _ _ = HNil
instance (TypeEq.TypeEq a b t,HDeleteAll' t b l a l') => HDeleteAll (HCons b l) a l' where
    hDeleteAll (HCons b l) a = hDeleteAll' (TypeEq.typeEq a b) b l a
...
instance (HContainsType m a t,HDeleteAllByHList' t a l m l') => HDeleteAllByHList (HCons a l) m l' where
    hDeleteAllByHList (HCons a l) m = hDeleteAllByHList' (hContainsType m a) a l m
...
instance HAfterFst HNil a HNil where
    hAfterFst _ _ = HNil
instance {-# OVERLAPPING #-} HList l => HAfterFst (HCons a l) a l where
    hAfterFst (HCons _ l) _ = l
instance {-# OVERLAPPABLE #-} HAfterFst l a l' => HAfterFst (HCons b l) a l' where
...
instance HBeforeFst HNil a HNil where
    hBeforeFst _ _ = HNil
instance (TypeEq.TypeEq a b t,HBeforeFst' t b l a l') => HBeforeFst (HCons b l) a l' where
    hBeforeFst (HCons b l) a = hBeforeFst' (TypeEq.typeEq a b) b l a
...
class HList r => HInject r a where
    hInject :: r -> a -> r
instance Ctrl.AssertFail (TypeNotPresent a) => HInject HNil a where
    hInject _ _ = hEmpty
...
class (HList l,HList l') => HReplaceFst l a b l' | l a b -> l' where
    hReplaceFst :: l -> a -> b -> l'
instance Ctrl.AssertFail (TypeNotPresent a) => HReplaceFst HNil a b HNil where
    hReplaceFst _ _ _ = HNil
instance (TypeEq.TypeEq a c t,HReplaceFst' t c l a b l') => HReplaceFst (HCons c l) a b l' where
    hReplaceFst (HCons c l) a b = hReplaceFst' (TypeEq.typeEq a c) c l a b
...
instance (HSelectAll r1 a r1Sa,
        ...
        HAppend r1Cr2 r1Dr2 r3) => HNotCommon r1 (HCons a r2) r3 where
    hNotCommon r1 (HCons a r2) = (\r1Sa r2Sa -> Logic.cond (hNull r1Sa) (HCons a r2Sa) hEmpty
        `hAppend` (hDeleteAll r1 a `hNotCommon` hDeleteAll r2 a)) (hSelectAll r1 a) (hSelectAll r2 a)
...
instance (HSelectAll r1 a r1Sa,
        ...
        HAppend r1Sa r1Sr2' r1Sr2) => HLeftCommon r1 (HCons a r2) r1Sr2 where
    hLeftCommon r1 (HCons a r2) = hSelectAll r1 a `hAppend` (hDeleteAll r1 a `hLeftCommon` hDeleteAll r2 a)
...
instance (HSelectAll r1 a r1Sa,
        HSelectAll r2 a r2Sa,
        HNull r1Sa t,
        HAppend r1Sa (HCons a r2Sa) r1Sr2,
        Logic.Conditional t HNil r1Sr2 r1Cr2,
        HDeleteAll r1 a r1Ea,
        HDeleteAll r2 a r2Ea,
        HCommon r1Ea r2Ea r1Ir2',
        HAppend r1Cr2 r1Ir2' r3) => HCommon r1 (HCons a r2) r3 where
    hCommon r1 (HCons a r2) = (\r1Sa -> Logic.cond (hNull r1Sa) hEmpty (r1Sa `hAppend` HCons a (hSelectAll r2 a))
        `hAppend` (hDeleteAll r1 a `hCommon` hDeleteAll r2 a)) (hSelectAll r1 a)
