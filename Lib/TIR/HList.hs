...
test1 :: HCons [Char] (HCons Char (HCons Bool (HCons (Maybe Char) (HCons Bool (HCons [[Char]] HNil)))))
...
class (HList r,HList r') => HMap f r r' | f r -> r' where
    hMap :: f -> r -> r'
instance {-# OVERLAPPABLE #-} HMap f HNil HNil where
    hMap _ _ = hEmpty
instance {-# OVERLAPPING #-} (Ctrl.Apply f a b,HMap f r r') => HMap f (HCons a r) (HCons b r') where
