...
instance Functor HtmlFragment where
    fmap f (HF m) = HF $ fmap f m 

instance Applicative HtmlFragment where
    pure x = HF $ pure x
    HF m <*> HF n = HF $ m <*> n
...
instance Functor HtmlTable where
    fmap f (HT m) = HT $ fmap f m 

instance Applicative HtmlTable where
    pure x = HT $ pure x
    HT m <*> HT n = HT $ m <*> n
...
instance Functor HtmlRow where
    fmap f (HR m) = HR $ fmap f m 

instance Applicative HtmlRow where
    pure x = HR $ pure x
    HR m <*> HR n = HR $ m <*> n
