...
class (RValues r l,HMap TypedToExpr l l',HOccursMany l' SqlExpression) => TypedToExpression r l l' | r -> l l' where
    typedToExpression :: r -> [SqlExpression]
instance (RValues r l,HMap TypedToExpr l l',HOccursMany l' SqlExpression) => TypedToExpression r l l' where
...
class (Record r, RMapValues SetTags r r) => Relabel r where
    relabel :: r -> Int -> r
instance (Record r, RMapValues SetTags r r) => Relabel r where
    relabel r i = rMapValues (SetTags i) r

------------------------------------------------------------------------------
-- Create a relation from a table schema

table :: (MonadState Int m,AttributeToTyped r r',TypedToExpression r' l l')
...
project :: (Monad m,RProject r p r',TypedToExpression r' l l')
...
extend :: (Monad m,HSnoc r (k,SqlTyped a b) r',TypedToExpression r' l l')
...
join :: (MonadState Int m,HAppend r r' r'',Relabel r,Relabel r',TypedToExpression r'' l l',IntersectionRowReps w w' w'')
...
union :: (MonadState Int m,Relabel r,TypedToExpression r l l',RValues r v,RValues r' v)
...
intersect :: (MonadState Int m,IntersectionRowReps w w' w'',Relabel r,
  TypedToExpression r l l',RValues r v,RValues r' v)
...
except :: (MonadState Int m,Relabel r,TypedToExpression r l l',RValues r v,RValues r' v)
  => Relation (SqlTypeRep w r) -> Relation (SqlTypeRep w' r') -> m (Relation (SqlTypeRep w r))
...
doUpdate :: (HNull u Logic.AllFalse,AttributeToTyped t t',
  RKeys u k,RProjectOrder2 t' k u,TypedToExpression u l l')
...
doInsertR :: (
    AttributeToTyped t t',
    RProjectOrder2 t' u u',
    TypedToExpression u' l l'
    ) => Relation r -> Table t -> u -> Query ()
...
instance (VerifyTables ts,ToColTypes t0) => VerifyTables (HCons (Table t0) ts) where
    verifyTables (HCons t0 ts) = do
        ...
        _ <- closeCursor
        verifyTable t0 (Set.fromList tableNameList)
        verifyTables ts
        return ()
...
getTableNames clist = do
    ...
    case status of
        SqlSuccess -> do
            ...
            tableName <- return (Map.findWithDefault Nothing "TABLE_NAME" row)
...
verifyTable :: ToColTypes r => Table r -> Set.Set SqlTableName ->  SqlHandle ()
verifyTable tab tableSet = case (map toLower $ sqlTableName tab) of
    tn  | tn `Set.member` tableSet -> do
            _ <- showColumns "" "" tn ""
...
vt :: Map.Map SqlColumnName String -> SqlTableName -> [Attr] -> SqlHandle ()
vt cm tn (a:cs) = do
    case Map.lookup (map toLower (attrName ad)) cm of
        Just t
...
getColTypeFM :: ColNameList -> SqlHandle (Map.Map SqlColumnName String)
getColTypeFM clist = do
    status <- fetch
    case status of
        SqlSuccess -> do
            row <- getRowAsFM clist
            colName <- return (Map.findWithDefault Nothing "COLUMN_NAME" row)
            colType <- return (Map.findWithDefault Nothing "DATA_TYPE" row)
            rows <- getColTypeFM clist
            case colName of
                Just cn -> case colType of
                    Just ct -> return (Map.insert (map toLower cn) ct rows)
                    _ -> return rows
                _ -> return rows
        _ -> return Map.empty
