...
module Lib.HTTP.Figo (getHandler) where
...
parseSelect :: Parser (Set.Set String)
parseSelect = do
    a <- ps
...
xorSelect :: String -> Set.Set String -> Set.Set String
xorSelect s sel = if Set.member s sel
    then Set.delete s sel
    else Set.insert s sel

showSelect :: Set.Set String -> String
showSelect s = concat (Set.toList s)

extractText :: URI -> DOM -> [String]
extractText _ [] = []
extractText uri d = case parse parseSelect (lookupDict uri "select") of
    Just (_,s) -> extractText' s d
    _ -> extractText' Set.empty d

extractText' :: (Set.Set String) -> DOM -> [String]
extractText' _ [] = []
extractText' select ((_,l):ns) = case l of
    ...
    extract ((_,e):ds) i [] = case e of
        (Text s) -> if Set.member (listToURI [i] "") select
    ...
    extract ((_,e):ds) i j@(j0:js) = case e of
        (Text s) -> if Set.member (listToURI (i:j) "") select
...
bodyToLink highlight uri src n = case parse parseSelect (lookupDict uri "select") of
    ...
    _ -> bodyToLink' highlight Set.empty uri src n
    
bodyToLink' :: Bool -> (Set.Set String) -> URI -> URI -> DOM -> DOM
bodyToLink' _ _ _ _ [] = []
bodyToLink' highlight select uri src ((_,l):ns) = case l of
    ...
    textToLink hl (d0@(d,e):ds) i [] = case e of
        (Text t) -> if isTextBlank t || (not hl)
            ...
            else (\ls -> (d,STag "A" (XmlAttribute ("href",[CharData $ show $ addToDict uri "select" (showSelect (xorSelect ls select))]):if Set.member ls select 
...
getHandler = return $ httpHandler figoHandler {- Application {
    mountPoint = ["figo"],
    handler = httpHandler figoHandler
} -}

figoHandler :: HttpHandler
figoHandler req _ = do
