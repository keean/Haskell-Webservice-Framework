...
httpURI uri = do
    ...
    e <- (do
        z <- httpParameters
        return (UriParameters $ Map.fromList (map (\(UriParameter u) -> u) z))) `mplus` return (uriParameters uri)
    return $ URI {
        uriProtocol=a,
        uriHost=b,
        uriPort=c,
...
httpHeader = do
    ...
    _httpFieldContent :: Parser String
    _httpFieldContent = do
...
httpRequest connection = do
    ...
    d <- many httpHeader
    _ <- httpEOL
    headers <- return $ HttpHeaders $ Map.fromList (map (\(HttpHeader z) -> z) d)
    return $ HttpRequest {
...
httpResponse = do
    ...
    return $ HttpResponse {
        ...
        responseHeaders = HttpHeaders $ Map.fromList (map (\(HttpHeader z) -> z) headers)
    }
