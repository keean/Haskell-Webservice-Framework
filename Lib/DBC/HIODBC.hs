...
module Lib.DBC.HIODBC(odbcConnect,connectToDb,safeAllocEnv,safeAllocConnect,safeConnect,safeAllocStmt,safeAllocBuffer,closeCursor,
...
safeAllocEnv doWith = ioBracket newEnv ioSqlDeleteEnv withEnv where
    ...
    newEnv = do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc in sqlNewEnv (" . shows s) ")")) 
        return env
    ...
    createEnv env = do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Bad status returned by sqlAllocEnv (" . shows s) ")"))
        return env
...
safeAllocConnect env doWith = ioBracket ( do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc in sqlNewConnect (" . shows s) ")"))
...
safeAllocStmt dbc doWith = ioBracket ( do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc in sqlNewStmt (" . shows s) ")"))
...
safeAllocBuffer bufSize doWith = case fromIntegral bufSize of
    sz -> ioBracket ( do
        buf <- ioSqlNewBuffer sz
        status <- ioSqlIsValidBuffer buf
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc in sqlNewBuffer (" . shows s) ")"))
...
numParams = ioBracket ( do
    ...
    _ <- ioIfFail status (\s -> fail ((showString " Could not malloc parPtr(" . shows s) ")"))
    return parPtr) (ioSqlDeleteSmallInt) (\parPtr -> do
        state <- getState
        status <- ioSqlNumParams (sqlStmt state) parPtr
        _ <- ioIfFail status (\s -> fail ((showString " Bad status returned by sqlNumParams (" . shows s) ")"))
...
numResultCols = ioBracket ( do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc colsPtr (" . shows s) ")"))
        return colsPtr) (ioSqlDeleteSmallInt) (\colsPtr -> do
            state <- getState
            status <- ioSqlNumResultCols (sqlStmt state) colsPtr
            _ <- ioIfFail status (\s -> fail ((showString " Bad status returned by sqlNumResultCols (" . shows s) ")"))
...
describeCol col = ioBracket ( do
    ...
    _ <- ioIfFail status (\s -> fail ((showString " Could not malloc colNamePtr (" . shows s) ")"))
    return colNamePtr) (ioSqlDeleteString) (\colNamePtr -> ioBracket ( do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc colTypePtr (" . shows s) ")"))
        return colTypePtr) (ioSqlDeleteSmallInt) (\colTypePtr -> ioBracket ( do
            ...
            _ <- ioIfFail status (\s -> fail ((showString " Could not malloc colPrecisionPtr (" . shows s) ")"))
            return colPrecisionPtr) (ioSqlDeleteUInteger) (\colPrecisionPtr -> ioBracket ( do
                ...
                _ <- ioIfFail status (\s -> fail ((showString " Could not malloc colScalePtr (" . shows s) ")"))
                return colScalePtr) (ioSqlDeleteSmallInt) (\colScalePtr -> ioBracket ( do
                    ...
                    _ <- ioIfFail status (\s -> fail ((showString " Could not malloc colNullablePtr" . shows s) ")"))
                    return colNullablePtr) (ioSqlDeleteSmallInt) (\colNullablePtr -> do
                        ...
                        status <- (ioSqlDescribeCol (sqlStmt state) (fromIntegral col) colNamePtr
                            ...
                            colNullablePtr)
                        _ <- ioIfFail status (\s -> fail ((showString " Bad status returned by sqlDescribeCol (" . shows s) ")"))
...
getDataAsString col = ioUnsafeInterleave $ do
    (s,dataAttr) <- ioBracket ( do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc dataAttrPtr (" . shows s) ")"))
...
getStmtError = ioBracket ( do
    ...
    _ <- ioIfFail status (\s -> fail ((showString " Could not malloc status buffer (" . shows s) ")"))
    return statBuf) (ioSqlDeleteString) (\statBuf -> ioBracket ( do
        ...
        _ <- ioIfFail status (\s -> fail ((showString " Could not malloc error buffer (" . shows s) ")"))
...
odbcConnect con m = do
    _ <- liftIO (installHandler sigPIPE Ignore Nothing)
...
sqlDo q = do
    ...
    _ <- prepare q
    do { _ <- execute; return () } `ioCatch` execError

sqlQuery :: String -> SqlHandle (ColNameList,[[Maybe String]])
sqlQuery q = do
    ioHPutStr stderr (shows q "\n")
    _ <- prepare q
    _ <- execute `ioCatch` execError
    cols <- getColNameList
    rows <- getRowsAsListOfLists' (length cols)
    return (cols,rows)

execError :: SomeException -> SqlHandle a
execError e = do
    errstr <- getStmtError
    fail ((shows e . showChar '\n' . showString errstr) "")
