...
getTableNames clist = do
    ...
    case status of
        SqlSuccess -> do
            ...
            tableName <- return $ fromJust (Map.findWithDefault Nothing "TABLE_NAME" row)
            rows <- getTableNames clist
            return (tableName:rows)
        _ -> return []

verifyTable :: Data a => SqlTable a -> Set.Set SqlTableName ->  SqlHandle ()
verifyTable tableName tableSet = case (sqlTableName tableName) of
    ...
    tableTypes = (getColumnTypes . sqlTableType) tableName
    ...
    toColTypes ((e,t):ts) = case e of
        ...
        _ -> ioUserException ((showString "Table "
            . showString (sqlTableName tableName)) " has bad attribute definition.")
    toColTypes _ = return []
...
vt :: Map.Map SqlColumnName String -> SqlTableName -> [(SqlColumnName,SqlType)] -> SqlHandle ()
vt cm tn ((cn,ct):cs) = do
 case Map.lookup (map toLower cn) cm of
    Just t
        | tp == ct -> vt cm tn cs
...
getColTypeFM clist = do
    ...
    case status of
        SqlSuccess -> do
            row <- getRowAsFM clist
            colName <- return $ map toLower $ fromJust (Map.findWithDefault Nothing "COLUMN_NAME" row)
            colType <- return $ fromJust (Map.findWithDefault Nothing "DATA_TYPE" row)
            rows <- getColTypeFM clist
            return (Map.insert colName colType rows)
        _ -> return Map.empty
...
type Query a = forall m . MonadIO m => StateT QState m a
type Select a = forall m . MonadIO m => StateT SelectState m a
