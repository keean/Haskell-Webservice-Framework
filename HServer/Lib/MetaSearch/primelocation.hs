...
getAgents = (do
	...
	case filterDOM (do alter tidyDOM; matchForm "ladder"; matchSelect getOptionsText "lst") src of
...
getInRange url key priceRange bedsRange postcode cookies = case priceRange of 
	Just (minPrice,maxPrice) -> case bedsRange of
		Just (minBeds,maxBeds) -> do
			...
			-- hPutStr stderr (generateXML src)
			(h,rdr) <- followRedirects "http-get://www.primelocation.com" hsrc cookies
			return $! rdr
		Nothing -> return []
	Nothing -> return []
...
doInserts _ _ [] _ = []
doInserts postcode cookies ((text,link):rs) agents = ( do
...
getPostcode prices beds pcode agents = do
	...
	extractProperties pcode cookies frm agents = case filterDOM ( do
...
getProperties _ [] _ _ = return []
getProperties _ _ [] _ = return []
getProperties prices beds postcodes@(p0:ps) agents = do
	hPutStr stderr ("{"++p0)
...
getUrlVars "" = []
getUrlVars s = case parseResult (many $ do
