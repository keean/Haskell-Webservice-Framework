...
data ElemType = Document|Header|Tag|Text|Comment|DTD|Close|EmptyTag|Null deriving (Eq,Show)
newtype Attribute = MkAttribute (String,String) deriving Show

data Elem = MkElem !(ElemType,String,[Attribute]) deriving Show
...
tagValue  = do
	(skipSpace . required . whileChar) isEqual
	a <- skipSpace value
	return a
...
_dtdTag  = do
	a <- skipSpace name
	b <- many tagArg
	(skipSpace . required . whileChar) isEndTag
...
_metaTag = do
	(skipSpace . required . whileChar) isPling
...
parseHeader = do
	...
	(skipBlanks . whileChar) isColon
	b <- (skipBlanks . untilChar) (\c -> c=='\n' || c=='\r')
	(skipBlanks . satisfy) isNewline
	return (a,b)
