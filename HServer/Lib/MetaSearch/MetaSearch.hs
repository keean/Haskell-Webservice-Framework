...
data Property = Property {
	propertyTimestamp :: Maybe ClockTime,
...
instance (NFData a,NFData b,NFData c,NFData d,NFData e,NFData f,NFData g,NFData h,NFData i,NFData j,NFData k) =>
		NFData (a,b,c,d,e,f,g,h,i,j,k) where
	rnf (a,b,c,d,e,f,g,h,i,j,k) = rnf a `seq` rnf b `seq` rnf c `seq` rnf d
	...
	case parseResult parsePostcodes (map toUpper as) of
		Ok pc _ -> do
			...
			t4 <- forkIO $
				...
				`Exception.catch` (\e -> hPutStr stderr $ (showString "propertyfinder: " . shows e) "\n")
				`finally` (do
...
pickRange (min,max) (r0@(p0,q0):rs@((p1,_):_)) = if (if p1 < 0 then 0 else p1) <= min
	...
	else let q' = if q0 <= 0 then maxInt else q0
		in (if p0 < 0 then 0 else p0,q'):if max > q' then pickMax max rs else []
...
checkRange mergeChannel pr@(pr0,pr1) bd@(bd0,bd1) p0 = case (propertyPrice p0,propertyBeds p0) of
	(Just p,Just b) -> if (pr0 <= p) && (pr1 >= p) && (bd0 <= b) && (bd1 >= b)
		then do
			t <- getClockTime
			writeChan mergeChannel (Just (p0 {propertyTimestamp = Just t}) `using` rnf)
			yield
		else return ()
	_ -> return ()
...
getAgents pcds = do
	...
	(case filterDOM (do
		...
		_ -> do
			maybeClose h
			return []) `Exception.catch` (\e -> do maybeClose h; throw e)
	where
...
getRanges = do
	...
	(case filterDOM (do 
			...
			rnf (a,c,f,minprice,maxprice,minbeds,maxbeds,minprice',maxprice',minbeds',maxbeds') `seq` return
...
getPLNext mergeChannel _ _ _ [] _ = return ()
...
getProperties _ _ _ _ [] _ _ _ _ = return ()
getProperties _ _ _ _ _ [] _ _ _ = return ()
getProperties mergeChannel pr bd prices beds postcodes@(p0:ps) agents rcode acf = do
	-- hPutStr stderr ("{"++p0)
...
getRMForm1 site = do
	...
	(do
		...
		return form) `Exception.catch` (\e -> do maybeClose h; throw e) 
...
getRMForm2 site (action,cookies,vars) pc = do
	...
	(do
		...
		return ((a2,setInputs c2 cookies,v2),beds,price)) `Exception.catch` (\e -> do maybeClose h; throw e)
...
getRMPostcodes mergeChannel _ _ [] _ _ = return ()
getRMPostcodes mergeChannel site form1@(action,cookies,vars) (p0:ps) pr bd = do
...
getPPForm site = do
	...
	(case filterDOM (do
		...
		_ -> do
			maybeClose h
			return Nothing) `Exception.catch` (\e -> do maybeClose h; throw e)
...
getPPPostcodes _ mergeChannel _ [] _ _ _ _ = return ()
...
getPFPostcodes :: Chan (Maybe Property) -> String -> ((URL,Cookies,FormVars),BedsRanges,PriceRanges)
	-> [Postcode] -> PriceRange -> BedsRange -> IO ()
getPFPostcodes mergeChannel _ _ [] _ _ = return ()
