...
isValidLogin user pass 
	-- | user=="bookamove" && pass=="khadoma1" = True 
	| user=="bruce" && pass=="test123" = True
	| user=="steve" && pass=="test321" = True
	| user=="Investor5" && pass=="cup21" = True
	| user=="Investor6" && pass=="phone2" = True
	| user=="Investor7" && pass=="money99" = True
	| user=="Investor8" && pass=="serge17" = True
	| user=="Investor9" && pass=="Larry12" = True
	| user=="Investor10" && pass=="eric88" = True
	| otherwise = False
...
maybeShowTime :: Maybe ClockTime -> IO ShowS
maybeShowTime (Just t) = do
	c <- toCalendarTime t
	return $ showString (calendarTimeToString c)
...
htmlMetaResults rsp (c0,c1,c2,c3) (p0:ps) state = attrP [khadomaStyle] $ do
				...
				case propertyDetails p0 of
					...
					Just l -> case searchParameters state of
						 Just (pcds,_,_,_,_) -> htmlLink l $ toTitle (toWords pcds)
							. toWords $ maybeShowString (propertyAddress p0) ""
						 _ -> htmlLink l $ toTitle (toWords "")
							. toWords $ maybeShowString (propertyAddress p0) ""
				htmlNobrText " - "
...
toTitle :: MonadHtml m => [String] -> [String] -> m () 
toTitle wl (s0:ss)
	| s0 `wordIn` wl = do
